...
- Node.js 20+
- pnpm 9+
- [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/)（已在 `devDependencies` 中，无需全局安装）
- Cloudflare 账号，需升级为 **Workers Paid 计划**（$5/月）

### Workers Paid 计划开通方式

本项目 bundle gzip 约 **5.3 MiB**，超出免费计划 3 MiB 上限，必须开通付费计划（10 MiB 上限）。

**这个计划不在 Cloudflare 普通套餐里**，需要单独开通：

1. 登录 [Cloudflare Dashboard](https://dash.cloudflare.com)
2. 左侧导航 → **Workers & Pages**
3. 页面右侧 **Usage Model** / **Subscription** → 点击 **Upgrade to Paid**
4. 按提示完成订阅（$5/月，含 10M 次请求/月）

或直接访问：`https://dash.cloudflare.com/<账户ID>/workers/plans`
...
|------|--------|------|
| 未压缩大小 | ~23 MiB | 无硬性限制 |
| gzip 大小 | ~5.3 MiB | 免费计划 3 MiB / 付费计划 10 MiB |
