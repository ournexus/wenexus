...
def search_node(
    ...
    sources = [
        Source(title=r.title, url=r.url, snippet=r.snippet, source_type=r.source_type)
        for r in results
    ]
...
def extraction_node(state: FactCheckState) -> FactCheckState:
    ...
    for source in state["search_results"]:
        if len(source.snippet) > 20:
            fact = Fact(
                content=source.snippet, claim=state["topic_title"], source=source
            )
...
def synthesis_node(state: FactCheckState) -> FactCheckState:
    """生成报告."""
    facts = state["extracted_facts"]
    cred_dist = {"high": 0, "medium": 0, "low": 0, "uncertain": 0}
    for f in facts:
        if f.source.source_type == "gov" or f.source.source_type == "academic":
            cred_dist["high"] += 1
        elif f.source.source_type == "news":
            cred_dist["medium"] += 1
        else:
            cred_dist["low"] += 1
